...
function usage(){
    echo "Usage: $0 [-e MODE - test/deploy] [-f FUNCTION_NAME] [-r RUNTIME] [-a ROLE] \
        ...
        [-l LAYERS AS A LIST IN QUOTES] [-v VPC SUBNETS AS A LIST IN QUOTES, USE \"INVALID\" FOR NO VPC] \
        [-s VPC SECURITY GROUPS AS A LIST IN QUOTES, USE \"INVALID\" FOR NO VPC][-o OUTPUT FILE FOR LAMBDA CONFIG] [-h HELP]"
    exit 2
}
